...
logger = setup_logger("train_visa_masterkey")

def train_single_model(X, Y, model_idx, sbox_idx, save_dir, epochs=30, batch_size=256):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Stratified Split
    try:
        X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size=0.1, random_state=42 + model_idx, stratify=Y)
    except ValueError:
        X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size=0.1, random_state=42 + model_idx)
...
def train_visa_ensemble(input_dir, output_dir, models_per_sbox=2, epochs=30):
    ...
    for sbox in range(8):
        ...
        logger.info(f"Training Visa Ensemble for S-Box {sbox+1}...")
        for i in range(models_per_sbox):
            train_single_model(X_curr, Y_curr, i, sbox+1, output_dir, epochs=epochs)

if __name__ == "__main__":
    train_visa_ensemble("Processed/Visa_Profiled", "Models/Ensemble_MasterKey_Visa_Dedicated", models_per_sbox=2, epochs=30)
