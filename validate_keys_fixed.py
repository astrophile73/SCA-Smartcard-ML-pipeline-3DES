...
try:
    ...
    for idx in range(min(5, len(meta))):
        ...
        # Check if valid
        kenc_valid = len(str(kenc).strip()) == 32
        kmac_valid = len(str(kmac).strip()) == 32
        kdek_valid = len(str(kdek).strip()) == 32
        
        print(f"\n  Row {idx}:")
        print(f"    Track2: {str(t2)[:20]}...")
        print(f"    KENC ({len(str(kenc))} chars): {'OK' if kenc_valid else 'FAIL'} {str(kenc)[:32]}")
        print(f"    KMAC ({len(str(kmac))} chars): {'OK' if kmac_valid else 'FAIL'} {str(kmac)[:32]}")
        print(f"    KDEK ({len(str(kdek))} chars): {'OK' if kdek_valid else 'FAIL'} {str(kdek)[:32]}")
